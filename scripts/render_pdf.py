...
1. Documento de Practica/TP con membrete institucional (el que se sube como
   ...
   se le agrega automaticamente encabezado (logo UTN + institucion) y pie de
   pagina (barra de color con el nombre de la materia + numero de pagina) en
   cada hoja, via header_template/footer_template de Playwright -- la unica
   forma de lograr un encabezado/pie que se repita en todas las paginas
   (el CSS de impresion normal de un navegador no soporta esto).
...
LOGO_DEFAULT = os.path.join(SKILL_DIR, "assets", "logo-utn-tup.jpg")
...
def render(entrada_html: str, salida_pdf: str, materia, logo_path: str) -> None:
    ...
    if con_membrete:
        if not os.path.exists(logo_path):
            sys.exit(f"No existe el logo: {logo_path}")
        header = HEADER_TEMPLATE.replace("__LOGO_B64__", _logo_base64(logo_path))
...
def main():
    ...
    ap.add_argument("--logo", default=LOGO_DEFAULT, help="Ruta al logo institucional (default: assets/logo-utn-tup.jpg)")
    args = ap.parse_args()

    render(args.entrada, args.salida, args.materia, args.logo)
