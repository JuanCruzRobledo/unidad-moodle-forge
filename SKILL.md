...
- Si **no existe** `.env`: es la primera vez que se usa esta skill en esta máquina. Avisale al usuario que puede correr `python scripts/configurar.py` (una sola vez, en su propia terminal — no se lo pidas vos por Bash si necesita tipear la API key, porque `getpass` no la va a ocultar bien en una terminal no interactiva) para activar automatizaciones y configuraciones opcionales (hoy: generación directa del Material de apoyo vía la API de Gamma, imagen del encabezado de los PDF con membrete — Práctica/Lectura/Evaluaciones/TPI, sin foto por default —, y subida automatizada de videos a YouTube). Si el usuario no quiere configurarlo ahora, seguí normalmente — todo funciona igual en modo manual/default, esto es puramente opcional.
- Si **ya existe** `.env`: no preguntes de nuevo. Cuando la Fase 2 llegue al paso de Material de apoyo, dejá que el script correspondiente (`scripts/gamma_generate.py`) lea `GAMMA_AUTOMATIZADO`/`GAMMA_API_KEY`/`GAMMA_THEME_ID` por su cuenta vía `python-dotenv` — vos nunca leas esos valores directo. Lo mismo para `scripts/render_pdf.py`, que lee `PDF_HEADER_LOGO` para saber si el encabezado de los PDF con membrete lleva una imagen (vacío/no configurado = sin imagen, el default) — ver `references/plantilla-pdf-practica.md` § Generar el PDF. Y para la subida de videos: si `YOUTUBE_AUTOMATIZADO=true`, `scripts/youtube_upload.py` toma sus credenciales solo (ver `references/automatizacion-subida-youtube.md` para el setup completo, que sí requiere pasos manuales del usuario en Google Cloud Console la primera vez — a diferencia de Gamma, esto no se resuelve solo con `configurar.py`).
...
5. **Videos de actividad** (si `incluir.videos_actividad`): los 3 videos de la actividad se generan **on-demand**, cuando el usuario pida avanzar esa actividad puntual — nunca en lote automático para varias actividades/unidades. Ver `references/automatizacion-videos-actividad.md` para el guion, la invocación de la skill `hyperframes`, y por qué se prefiere ese mecanismo al video nativo de NotebookLM. Marcá `videos[K].guion_status` y `render_status` en `estado.yml` a medida que avanza cada video. Una vez renderizado el `.mp4`, la subida a YouTube tiene el mismo patrón que Gamma: por default el usuario lo sube a mano y te devuelve la URL real (`videos[K].url_subida` + `url`); existe un camino opcional automatizado vía la YouTube Data API v3 si el usuario ya configuró credenciales — ver `references/automatizacion-subida-youtube.md` (incluye la regla del título real que sube a YouTube, distinta del texto del link en el HTML, y por qué la organización en playlists nunca se asume).
...
- **NotebookLM no tiene API — nunca se automatiza vía API, y nunca se inventa un link.** Sin browser automation disponible, el entregable es el guion fuente para que el usuario lo suba a mano y te pase la URL real. Si el usuario SÍ tiene una herramienta de browser automation (ej. `claude-in-chrome`), se puede automatizar el flujo completo (crear notebook, subir fuentes, generar los 5 materiales de Studio, compartir, pegar el link) — ver `references/notebooklm-guion.md`. En ambos casos la regla no negociable es la misma: todo link que termine pegado en el HTML tiene que haber sido verificado en vivo (abierto y confirmado real) antes de pegarlo — nunca un link inventado ni asumido.
...
| `scripts/configurar.py` | Setup interactivo de esta máquina (Fase -1): crea `.env`, pide la API key de Gamma con `getpass` (nunca por el chat), la imagen opcional del encabezado de los PDF (`PDF_HEADER_LOGO`, sin foto por default) y activa/desactiva el resto de automatizaciones opcionales |
...
| `scripts/render_pdf.py` | Convierte un HTML ya confirmado a PDF fiel (Playwright + Chromium); con `--materia` agrega membrete institucional (header/footer repetidos) — usado tanto por la Práctica como por Lectura PDF. La imagen del encabezado es opcional (default: sin foto); se configura de forma persistente vía `PDF_HEADER_LOGO` en `.env` o puntualmente con `--logo <ruta>` |
...
| `references/automatizacion-subida-youtube.md` | Opcional: setup de credenciales OAuth y uso de `scripts/youtube_upload.py` para subir un `.mp4` ya renderizado a YouTube vía la API — incluye la regla del título real y por qué las playlists nunca se asumen (Fase 2) |
| `scripts/youtube_upload.py` | Opcional (requiere credenciales OAuth propias, activado vía `YOUTUBE_AUTOMATIZADO=true` en `.env`): sube un video ya renderizado a YouTube y opcionalmente lo agrega a una playlist — ver `references/automatizacion-subida-youtube.md` |
